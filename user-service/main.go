...
import (
	...
	"github.com/mahendraintelops/first-rest-server-project-sqlite-gorm/user-service/config"
	restcontrollers "github.com/mahendraintelops/first-rest-server-project-sqlite-gorm/user-service/pkg/rest/server/controllers"
...
func main() {
	...
	addressController, err := restcontrollers.NewAddressController()
	if err != nil {
		log.Errorf("error occurred: %v", err)
		os.Exit(1)
	}
	...
	v1 := router.Group("/v1")
	{
		...
		v1.POST("/addresses", addressController.CreateAddress)

		v1.GET("/addresses/:id", addressController.FetchAddress)

		v1.PUT("/addresses/:id", addressController.UpdateAddress)

		v1.DELETE("/addresses/:id", addressController.DeleteAddress)
